...
describe("AsyncJobManager", () => {
	...
	test("unwatched jobs trigger delivery after unwatch", async () => {
		...
		// After unwatching, there should be a pending delivery
		expect(manager.hasPendingDeliveries()).toBe(true);
		await manager.drainDeliveries({ timeoutMs: 500 });

		// Now should have triggered onJobComplete
		expect(completions).toEqual([jobId]);
	});

	test("watched jobs that fail do not trigger onJobComplete", async () => {
		const completions: string[] = [];
		const manager = new AsyncJobManager({
			onJobComplete: async jobId => {
				completions.push(jobId);
		...
		// Should NOT have called onJobComplete even for failed jobs
		expect(completions).toHaveLength(0);
		expect(manager.getJob(jobId)?.status).toBe("failed");
	});

	test("dispose clears watched jobs", async () => {
		const manager = new AsyncJobManager({
			onJobComplete: async () => {},
		});
		...
		// Before dispose, job is watched - delivery should be suppressed
		expect(manager.hasPendingDeliveries()).toBe(false);

		await manager.dispose();

		// After dispose, everything should be cleared
		expect(manager.getJob(jobId)).toBeUndefined();
	});

	test("multiple jobs watched together only prevent their own delivery", async () => {
		const completions: string[] = [];
		const manager = new AsyncJobManager({
			onJobComplete: async jobId => {
				completions.push(jobId);
		...
		const watchedJobId = manager.register("bash", "watched job", async () => "watched output");
		const unwatchedJobId = manager.register("bash", "unwatched job", async () => "unwatched output");
		...
		// Only the unwatched job should have triggered delivery
		expect(completions).toEqual([unwatchedJobId]);
	});
});
