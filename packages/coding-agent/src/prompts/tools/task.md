...
- **Frozen execution:** Once a mutating contract and its exact file ownership are settled, set `mode: "execute"` with non-empty workspace-relative `writes`. OMP accepts completion only after a declared file changes, prompts once to correct an artifact-free yield, then fails. Never use this mode for research or review.
